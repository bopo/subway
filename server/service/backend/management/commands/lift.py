...
class BackendStaticFilesHandler(StaticFilesHandler):
    def _should_handle(self, path):
        return path.startswith((settings.STATIC_URL, settings.MEDIA_URL))

    def get_response(self, request):
        response = super(BackendStaticFilesHandler, self).get_response(request)
...
def banner():
    ...
    db_name = "%s%s" % (db_name[:1].upper(), db_name.replace("sql", "SQL").replace("db", "DB")[1:])
    ...
    lines = ("""
     ...
     .p'                `q.
    .d'                   `b.
   .d'                     `b.   * LUBOOK %(lubook_version)s
   ::                       ::   * Django %(django_version)s
   ::      L U B O O K      ::   * Python %(python_version)s
...
class Command(runserver.Command):
    ...
    def get_handler(self, *args, **options):
        ...
        if settings.DEBUG or options["insecure_serving"]:
            handler = BackendStaticFilesHandler(handler)
